...
class ArticlesConverter {
  ...
  def toArticles(xml: String, language: String): Seq[Article] = {

    val rootElement = scala.xml.XML.loadFile(xml)

    val articlesNode = (rootElement \ "articles").map {
      articles =>
        val articleList = (articles \ "article").map {
          article =>
            ...
            val topicsNode = (article \ "topics").map {
              topics =>
                val topicList = (topics \ "topic").map {
                  topic =>
                    val name = (topic \ "name").text
                    val subtopicsNode = (topic \ "subtopics").map {
                      subtopics =>
                        val subtopicsList = (subtopics \ "subtopic").map {
                          subtopic =>
                            subtopic.text
                        }
            ...
            Article(id, link, title, body, rank, updated, topicsNode.toList(0), language)
        }
